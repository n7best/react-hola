...
var YandexTranslator = function () {
  ...
  createClass(YandexTranslator, [{
    key: 'translateJSON',
    value: function translateJSON(json, src, target, callback) {
      var data = [];
      iterate(json, function (text) {
        var regMustache = /\{\{(.*?)\}\}/g;

        if (text.match(regMustache)) {
          data.push(text.replace(regMustache, function (match, p1) {
            return 'NO_TRANSLATION_' + p1.trim();
          }));
        } else {
          data.push(text);
        }
      ...
      this.translate(data, src, target, function (translations) {
        var newLangData = JSON.parse(JSON.stringify(json));
        var index = 0;

        iterate(newLangData, function () {
          var trans = translations[index];
          index += 1;

          if (trans.indexOf('NO_TRANSLATION_') > -1) {
            trans = trans.replace(/NO_TRANSLATION_(\w+)/g, function (match, p1) {
              return '{{ ' + p1 + ' }}';
            });
          }
          return trans;
...
function translatable(key) {
  return function (component) {
    return withTranslation(component, key);
  };
}
